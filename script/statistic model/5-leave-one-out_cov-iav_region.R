...
for(s in seasons){
  ...
  save(preds, file = paste0("output_pred/preds_cov_iav_",region[j],"/preds_season_", s_char,  ".RData"))
...
ggplot(df) + 
  ...
  scale_colour_identity(name = "",
                        ...
                        guide = "legend") +
  theme_bw() + 
  ylab("SARS-CoV-2 positivity rate (standardized, log)")+
  xlab(NULL)+
...
#######
write.csv(df,paste0("output_pred/preds_cov_iav_",region[j],"/cov-data.csv"))



###combine 5 results############################################
df1<-read.csv("output_pred/preds_cov_iav_Denmark/cov-data.csv")
df2<-read.csv("output_pred/preds_cov_iav_Ireland/cov-data.csv")
df3<-read.csv("output_pred/preds_cov_iav_Portugal/cov-data.csv")
df4<-read.csv("output_pred/preds_cov_iav_Slovenia/cov-data.csv")
df5<-read.csv("output_pred/preds_cov_iav_England/cov-data.csv")
...
p<-ggplot(finaldf) + 
  ...
  geom_line(aes(x = yearweek, y = Y_COV_1, col = "#499894",group=subregion)) +
  scale_colour_identity(name = "",
                        breaks = c("#499894", "#D37295"),
                        labels = c("Observed", "Posterior predicted"),
                        guide = "legend") +
...
ggsave("output_pred/cov_iav_5countries.tiff", p, width=20, height=15, units="cm", dpi=300, compression = "lzw")
