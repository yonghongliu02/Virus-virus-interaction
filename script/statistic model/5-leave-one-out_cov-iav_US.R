...
for(s in seasons){
  ...
  nsamples <- 1000  # 后验抽样次数
   ...
   save(preds, file = paste0("output_pred/preds_cov_iav/preds_season_", s_char,  ".RData"))
...
p<- ggplot(df) + 
 ...
 ggsave("output_pred/cov_iav_US.tiff", p, width=20, height=15, units="cm", dpi=300, compression = "lzw")
