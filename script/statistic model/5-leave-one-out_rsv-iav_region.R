...
for(s in seasons){
  ...
  save(preds, file = paste0("output_pred/preds_rsv_iav_",region[j],"/preds_season_", s_char, ".RData"))
...
#######
write.csv(df,paste0("output_pred/preds_rsv_iav_",region[j],"/rsv-data.csv"))



###combine 5 results
df1<-read.csv("output_pred/preds_rsv_iav_Denmark/rsv-data.csv")
df2<-read.csv("output_pred/preds_rsv_iav_Ireland/rsv-data.csv")
df3<-read.csv("output_pred/preds_rsv_iav_Portugal/rsv-data.csv")
df4<-read.csv("output_pred/preds_rsv_iav_Slovenia/rsv-data.csv")
df5<-read.csv("output_pred/preds_rsv_iav_England/rsv-data.csv")
...
p
ggsave("output_pred/rsv_iav_5countries.tiff", p, width=20, height=15, units="cm", dpi=300, compression = "lzw")
