...
for (j in 1:3) {
  ...
  for (i in 1:2) {
    ...
    #1.data
    df.us<-read_excel(paste0("output_new/select_model_us_",y[j],"/coef.list.xlsx"),sheet=x[i])
    df.slovenia<-read_excel(paste0("output_new/select_model_region_",y[j],"/coef.list.Slovenia.xlsx"),sheet=x[i])
   # df.hk<-read_excel(paste0("output_new/select_model_region_",y[j],"/coef.list.HK.xlsx"),sheet=x[i])
    df.denmark<-read_excel(paste0("output_new/select_model_region_",y[j],"/coef.list.Denmark.xlsx"),sheet=x[i])
    df.ireland<-read_excel(paste0("output_new/select_model_region_",y[j],"/coef.list.Ireland.xlsx"),sheet=x[i])
    df.portugal<-read_excel(paste0("output_new/select_model_region_",y[j],"/coef.list.Portugal.xlsx"),sheet=x[i])
    df.england<-read_excel(paste0("output_new/select_model_region_",y[j],"/coef.list.England.xlsx"),sheet=x[i])
    # df.usall<-read_excel(paste0("output_new/select_model_region_",y[j],"/coef.list.Usall.xlsx"),sheet=x[i])
    ...
    vcov.list[[1]]=read_excel(paste0("output_new/select_model_us_",y[j],"/vcov.list.xlsx"),sheet=x[i])
    vcov.list[[2]]=read_excel(paste0("output_new/select_model_region_",y[j],"/vcov.list.Slovenia.xlsx"),sheet=x[i])
   # vcov.list[[3]]=read_excel(paste0("output_new/select_model_region_",y[j],"/vcov.list.HK.xlsx"),sheet=x[i])
    vcov.list[[3]]=read_excel(paste0("output_new/select_model_region_",y[j],"/vcov.list.Denmark.xlsx"),sheet=x[i])
    vcov.list[[4]]=read_excel(paste0("output_new/select_model_region_",y[j],"/vcov.list.Ireland.xlsx"),sheet=x[i])
    vcov.list[[5]]=read_excel(paste0("output_new/select_model_region_",y[j],"/vcov.list.Portugal.xlsx"),sheet=x[i])
    vcov.list[[6]]=read_excel(paste0("output_new/select_model_region_",y[j],"/vcov.list.England.xlsx"),sheet=x[i])
    #vcov.list[[7]]=read_excel(paste0("output_new/select_model_region_",y[j],"/vcov.list.Usall.xlsx"),sheet=x[i])
    ...
    #加协变量2
    mvall.all.new <- mvmeta(df.coef~1+pop_density+above65+SDI ,vcov.list,
                            data=meta.new,
                            # control=list(showiter=F,igls.iter=10000)，
                            method=method)
    
    #加协变量2
    mvall.all.pop <- mvmeta(df.coef~1+pop_density ,vcov.list,
                            data=meta.new,
                            # control=list(showiter=F,igls.iter=10000)，
                            method=method)
    ...
    mvall.list[["0-all.new"]]<-mvall.all.new
    mvall.result<-as.data.frame(cbind(variable=NA,Q=NA,df=NA,p=NA,I2=NA,AIC=NA))
    ...
    for (z in 1: length(mvall.list)) {
      ...
      mvall.result[z,"AIC"]=AIC(mvall.list[[z]])
   ...
   # levels<-pretty(zz,20)
    ...
    all.plot<-ggplot(all.df, aes(x=lag,y=(Var2),  z=Freq)) +
      ...
      theme(axis.text.x = element_text(size = 8, color="black",vjust = 0.5))+
      theme(axis.text.y = element_text(size = 8, color="black",vjust = 0.5))+
      labs(title = paste0("RR of ",unique(all.df$y))) + 
      labs(fill =NULL) +
      theme(axis.title.x = element_text(size = 8, color="black",vjust = 0.5))+
      theme(axis.title.y = element_text(size = 8, color="black",vjust = 0.5))+
      ...
      theme(legend.title = element_text(size =8),
            ...
            legend.key.height = unit(0.4, "cm") ) # Adjusted height)
   ...
   cumulative.df.list[[paste0(y[j],"-",x[i])]]<-  cumulative.df
 ...
 print( cumulative.df[which.min(cumulative.df$allrr),])
  ...
  best.plot<- ggplot( specify.df) + 
    ...
    xlab("Lag")+
    theme_classic()+
    theme(axis.text.x = element_text(size = 5, color="black",vjust = 0.5))+
    theme(axis.text.y = element_text(size = 5, color="black",vjust = 0.5))+
    theme(plot.title = element_text(hjust = 0.5)) + 
...
arranged_plot<-ggarrange(
  allplot.list[[1]], cumuplot.list[[1]], 
  allplot.list[[2]], cumuplot.list[[2]], 
  allplot.list[[3]], cumuplot.list[[3]], 
  allplot.list[[4]], cumuplot.list[[4]], 
  allplot.list[[5]], cumuplot.list[[5]],
  allplot.list[[6]], cumuplot.list[[6]], 
  
  ncol = 2,  nrow = 6, align = 'hv',hjust=-2,
  labels = c("A", "B", "C",
             "D", "E", "F",
             "G", "H", " I",
             "J", "K", "L"))
...
arranged_plot
#ggsave("../figure_new/fig_all_meta.pdf", plot = arranged_plot, width=16, height=36, units="cm")


write_xlsx(mvall.result.list, path = "../figure_new/all_meta.xlsx")
