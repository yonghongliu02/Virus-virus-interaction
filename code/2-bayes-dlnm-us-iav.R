...
########################1. load map###########################
library(sf)
map <- read_sf("../data/gadm41_USA_shp/gadm41_USA_1.shp") 
region<-read_excel("../data/region_us.xlsx")
...
##add popu
popu=read_excel("../data/meta.xlsx")
...
##time change
#df$T2 <- ifelse(df$T1>=1&df$T1<=39, df$T2- 1, df$T2)
#timedate=cbind(T1=c(40:52,1:39),T1new=c(1:52))
#df=merge(df,timedate,by="T1",all.x=T)

#df<- df %>% 
#  select(-"T1") 

#setnames(df,old="T1new",new="T1")
...
models <- lapply(1:length(formulas), 
                 function(i) {
                   ...
                   save(model, file = paste0("../output_new/select_model_us_iav/iav_", lab[i],".RData"))})
...
for(i in 1:length(formulas)){
  ...
  load(paste0("../output_new/select_model_us_iav/iav_",lab[i],".RData"))
...
models <- lapply(1:length(formulas.best), 
                 function(i) {
                   ...
                   save(model, file = paste0("../output_new/select_model_us_iav/iav_", lab.best[i],".RData"))})
...
for(i in 1:length(formulas.best)){
  ...
  load(paste0("../output_new/select_model_us_iav/iav_",lab.best[i],".RData"))
...
######################## 9. compare MAE###########################
#basemodel
load("../output_new/select_model_us_iav/iav_basemodel.RData")
basemodel <- model
#best model
load("../output_new/select_model_us_iav/iav_model1.1.RData")
...
######################## 10. cross validation###########################
 ...
 for(i in 1:nrow(time)){
   ...
   save(preds, file = paste0("../output_new/preds_iav_rsv/preds_",2020 + time[i,2], "_", time[i,1], ".RData"))
 ...
 for(i in 1:nrow(time)){    
   ...
   load(paste0("../output_new/preds_iav_rsv/preds_",2020 + time[i,2],"_",time[i,1],".RData"))
 ...
 data$subregion=factor(data$subregion,
...
table0.best.out$load=c("../output_new/select_model_us_iav/iav_model1.1.RData",
                       "../output_new/select_model_us_iav/iav_model1.2.RData",
                       "../output_new/select_model_us_iav/iav_model1.3.RData",
                       "../output_new/select_model_us_iav/iav_model1.4.RData")
...
filled.contour(x,(y),z,
               xlab = "Lag", ylab = table0.best.out[i,"Model"], main = "IAV",
...
########export
library(writexl)
write_xlsx(table2d.list, "../output_new/select_model_us_iav/table2d.list.xlsx")
write_xlsx(coef.list, "../output_new/select_model_us_iav/coef.list.xlsx")
write_xlsx(vcov.list, "../output_new/select_model_us_iav/vcov.list.xlsx")
