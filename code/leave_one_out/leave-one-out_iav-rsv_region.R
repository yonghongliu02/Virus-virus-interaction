...
for(i in 1:nrow(df)){
  ...
  save(preds, file = paste0("../output_new/preds_iav_rsv_",region[j],"/preds_",2020 + df[i,"T2"], "_",  df[i,"T1"], ".RData"))
...
for(i in 1:nrow(df)){    
  ...
  load(paste0("../output_new/preds_iav_rsv_",region[j],"/preds_",2020 +  df[i,"T2"],"_", df[i,"T1"],".RData"))
...
#######
write.csv(df,paste0("../output_new/preds_iav_rsv_",region[j],"/iav-data.csv"))
...
ggsave("../figure/iav_rsv_5countries.tiff", p, width=20, height=15, units="cm", dpi=300, compression = "lzw")
